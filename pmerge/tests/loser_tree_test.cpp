...
void LoserTreeTest(const std::array<std::vector<pmerge::IntermediateInteger>, ResourcesAmount>& vectors){
  auto result = SimpleMultiwayMerge(vectors);
  auto tree{pmerge::multi_way::MakeLoserTree<pmerge::common::VectorResource, ResourcesAmount>(vectors)};
...
TYPED_TEST(LoserTreeSuite, same_numbers){
  ...
  using TreeType = pmerge::multi_way::LoserTree<pmerge::common::VectorResource, 1<<depth >;
  std::array<std::vector<pmerge::IntermediateInteger>, 1<<depth> data = {};
  for(size_t k = 0; auto& vec: data){
    vec = pmerge::common::FromDataAndIndex(std::views::repeat(0, 16) | std::ranges::to<std::vector<uint64_t>>(), std::bitset<depth>{k++});
  }
  LoserTreeTest(data);
}
...
TYPED_TEST(LoserTreeSuite, inc_by_one){
  ...
  for(size_t num : std::views::iota(0, 500)) {
    size_t index = gen();
    data[index].push_back(pmerge::PackFrom(num << (depth+1), std::bitset<depth>{ index}));
  }
  LoserTreeTest(data);
}

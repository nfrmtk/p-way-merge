...
inline std::vector<int64_t> SimpleMultiwayMerge(
    const std::ranges::range auto& nums) requires std::same_as<std::vector<int64_t>,std::ranges::range_value_t<decltype(nums)>> {
...
inline auto MakeRandomGenerator(uint64_t low, uint64_t high, uint64_t seed = 123){
  struct Generator{

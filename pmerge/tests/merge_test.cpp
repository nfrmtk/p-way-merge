...
TEST(SimdInregisterMerge, MergeSimple) {
  ...
  ASSERT_PRED2(pmerge::simd::CompareRegistersEqual, reg_even,
               pmerge::simd::MakeFrom(1, 2, 3, 4));
  ASSERT_PRED2(pmerge::simd::CompareRegistersEqual, reg_odd,
               pmerge::simd::MakeFrom(5, 6, 7, 8));
}
...
TEST(SimdInregisterMinMax, Simple) {
  ...
  ASSERT_PRED2(pmerge::simd::CompareRegistersEqual, reg_smaller,
               pmerge::simd::MakeFrom(1, 1, 2, 2))
      << pmerge::simd::ToString(reg_bigger);
  ASSERT_PRED2(pmerge::simd::CompareRegistersEqual, reg_bigger,
               pmerge::simd::MakeFrom(3, 3, 4, 4))
      << pmerge::simd::ToString(reg_bigger);
}
